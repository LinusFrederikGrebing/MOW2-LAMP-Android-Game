...
class DoublePoints(context: Context, screenHeight: Int, screenWidth: Int, x: Double, y: Double) :
    ItemController() {
    // creates the associated model
    override var itemModel: ItemModel = DoublePointsModel(
        screenHeight,
        screenWidth,
    ...
    init {
        itemModel.unsizedBmp =
            BitmapFactory.decodeResource(context.resources, R.drawable.doublepoints)
