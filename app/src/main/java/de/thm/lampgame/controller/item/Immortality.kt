...
class Immortality(context: Context, screenHeight: Int, screenWidth: Int, x: Int, y: Int ) : ItemController() {
    override var itemModel: ItemModel = ImmortalityModel(screenHeight, screenWidth, (0.1*screenHeight).toInt(),(0.05*screenWidth).toInt(), x,y)
    init {
        itemModel.unsizedBmp = BitmapFactory.decodeResource(context.resources, R.drawable.immortality_highlight)
